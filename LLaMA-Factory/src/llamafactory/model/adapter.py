...
def _setup_lora_tuning(
    ...
    # 如果提供了adapter_name_or_path
    if model_args.adapter_name_or_path is not None:
        is_mergeable = True # 初始化<是否可以合并>标记
        # 如果模型是量化模型，且使用了LoRA或OFT微调，且适配器数量大于1，
        # 则无法合并适配器，因为合并后的模型可能无法正常工作
        if getattr(model, "quantization_method", None):  # merge lora in quantized model is unstable
            assert len(model_args.adapter_name_or_path) == 1, "Quantized model only accepts a single adapter."
            is_mergeable = False
        # 如果使用了DeepSpeed ZeRO-3，且适配器数量大于1，
        # 则无法合并适配器，因为合并后的模型可能无法正常工作
        ...
        if model_args.use_unsloth:
            ...
            is_mergeable = False
        # 若模型可训练且未创建新适配器，或适配器不可合并，
        # 则将最后一个适配器设为恢复训练的适配器，其他适配器合并为一个
        if (is_trainable and not finetuning_args.create_new_adapter) or (not is_mergeable):
            adapter_to_merge = model_args.adapter_name_or_path[:-1]
            adapter_to_resume = model_args.adapter_name_or_path[-1]
        # 否则，指定要合并的适配器为adapter_name_or_path
        else:
            adapter_to_merge = model_args.adapter_name_or_path # 已经转化为列表
        ...
        # 若提供adapter_to_resume，则恢复训练该适配器
    ...
    # 未提供adapter_to_resume，则创建新的LoRA适配器
...
def init_adapter(
    ...
    # 如果设置为LoRA或OFT微调，初始化LoRA或OFT适配器
