...
def load_model(
    ...
    # 如果到该步model未加载且不是懒加载模式，准备加载模型!
    if model is None and not lazy_load:
        init_kwargs["config"] = config
        init_kwargs["pretrained_model_name_or_path"] = model_args.model_name_or_path
        # 单独处理混合深度模型（mixture_of_depths）的加载
