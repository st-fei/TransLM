...
if is_mcore_adapter_available() and is_env_enabled("USE_MCA"):
    ...
    _TRAIN_MCA_ARGS = [ModelArguments, DataArguments, McaTrainingArguments, FinetuningArguments, GeneratingArguments, CustomArguments]
    _TRAIN_MCA_CLS = tuple[
        ModelArguments, DataArguments, McaTrainingArguments, FinetuningArguments, GeneratingArguments, CustomArguments
    ]
else:
    _TRAIN_MCA_ARGS = []
    _TRAIN_MCA_CLS = tuple()
...
def read_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> Union[dict[str, Any], list[str]]:
    ...
    # <read_args function>主要作用是从命令行或配置文件中获取参数
    # 如果提供了args参数，直接返回args
    if args is not None:
        return args
    # 若命令行第一个参数为yaml or yml配置文件
    if sys.argv[1].endswith(".yaml") or sys.argv[1].endswith(".yml"):
        # 利用OmegaConf.from_cli加载yaml or yml配置文件
        override_config = OmegaConf.from_cli(sys.argv[2:])
        dict_config = OmegaConf.load(Path(sys.argv[1]).absolute())
        return OmegaConf.to_container(OmegaConf.merge(dict_config, override_config))
    # 若命令行第一个参数为json配置文件
    elif sys.argv[1].endswith(".json"):
        # 利用OmegaConf.from_cli加载json配置文件
        override_config = OmegaConf.from_cli(sys.argv[2:])
        dict_config = OmegaConf.load(Path(sys.argv[1]).absolute())
        return OmegaConf.to_container(OmegaConf.merge(dict_config, override_config))
    # 否则，直接返回命令行传入的参数
...
def _parse_train_mca_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_MCA_CLS:
    ...
    allow_extra_keys = is_env_enabled("ALLOW_EXTRA_ARGS")
    model_args, data_args, training_args, finetuning_args, generating_args, custom_args = _parse_args(
...
def get_eval_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _EVAL_CLS:
    model_args, data_args, eval_args, finetuning_args, custom_args = _parse_eval_args(args)
    ...
    return model_args, data_args, eval_args, finetuning_args, custom_args
